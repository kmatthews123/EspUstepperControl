...
//Set LED brightness using values from 0-255, the closer to 255 the brigher the LED will be
    ...
    if(isNumber(body) && body.toInt() == constrain(body.toInt(), 0, 255)){
      ...
      WebSerial.println("invalid Input Value for LED2 must be between 0 - 255, you entered: " + body);
    }
 }
 else if(d == "hA"){
  WebSerial.println("'B' = motor 1 syntax is: ");
  WebSerial.println("B,maxAcceleration,maxVelocity,directionOfMotor,stopValueOfMotor");
  ...
  WebSerial.println("'LED2' Header denotes backlight LED being called, the expected syntax is as follows: ");
  WebSerial.println("LED2,valueOfBrightness");
  ...
  WebSerial.println("Accepted Headers are 'B' , 'LED2'");
  WebSerial.println("enter 'hB' for syntax of 'B'");
  WebSerial.println("enter hL for syntax of 'LED2'"); 
  }
}
